...
public class ExcelUtil {
    final static LoggerControler log = getLogg(ExcelUtil.class);
    //定义一个excel
   ...
   /*//插入数据到指定行
    ...
    //指定列插入数据
    public static void writeInfoToExcelByCell(String filePath, int cell, int row, ArrayList<ArrayList<String>> infos) {
//        判断文件是否存在
        if (false) {
            ExcelUtil.createExcel1(filePath);
        }
        ...
        try {
            ...
            cell1.getCellType();
                cell1.setCellValue(infos.get(0).get(0));
